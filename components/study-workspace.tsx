...
export function StudyWorkspace() {
  ...
  function renderModeForm() {
    switch (mode) {
      ...
      case "summarize":
        return (<div className="study-form-grid"><label className="study-full-field">Material to summarize<textarea value={learningMaterial} onChange={(event) => setLearningMaterial(event.target.value)} placeholder="Paste notes, a chapter, or any text you want summarized for revision..." rows={6} /></label><label>Summary style<select value={summaryStyle} onChange={(event) => setSummaryStyle(event.target.value)}>{summaryStyles.map((item) => <option key={item}>{item}</option>)}</select></label><label>Subject <span className="study-optional">(from your profile)</span><input value={subject} onChange={(event) => setSubjectOverride(event.target.value)} placeholder="e.g. Biology" /></label><label>Topic <span className="study-optional">(optional)</span><input value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="e.g. Photosynthesis notes" /></label><p className="study-field-hint study-full-field">You can also attach a document inside Chat after your session opens.</p></div>);
      case "plan":
        return (<div className="study-form-grid"><label className="study-full-field">Learning goal<textarea value={goal} onChange={(event) => setGoal(event.target.value)} placeholder="What do you want to achieve? e.g. Master Chemistry S4 by the end of term" rows={3} /></label><label>Subject <span className="study-optional">(from your profile)</span><input value={subject} onChange={(event) => setSubjectOverride(event.target.value)} placeholder="e.g. Chemistry" /></label><label>Topics <span className="study-optional">(optional)</span><input value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="e.g. Moles, Acids, Electrolysis" /></label><label>Available study time<input value={availableStudyTime} onChange={(event) => setAvailableStudyTime(event.target.value)} placeholder="e.g. 45 minutes per day" /></label><label>Plan length<select value={String(planDays)} onChange={(event) => setPlanDays(Number(event.target.value))}>{[7, 14, 30, 60, 90].map((days) => <option key={days} value={days}>{days} days</option>)}</select></label><label>Preferred schedule <span className="study-optional">(optional)</span><input value={preferredSchedule} onChange={(event) => setPreferredSchedule(event.target.value)} placeholder="e.g. Weekday evenings, Saturday mornings" /></label></div>);
      case "check":
        return (<div className="study-form-grid"><label className="study-full-field">Question or problem<textarea value={learningMaterial} onChange={(event) => setLearningMaterial(event.target.value)} placeholder="What question are you answering?" rows={3} /></label><label>Subject <span className="study-optional">(from your profile)</span><input value={subject} onChange={(event) => setSubjectOverride(event.target.value)} placeholder="e.g. Mathematics" /></label><label className="study-full-field">Your answer<textarea value={learnerAnswer} onChange={(event) => setLearnerAnswer(event.target.value)} placeholder="Write your answer here..." rows={5} /></label><label className="study-full-field">Marking scheme or context <span className="study-optional">(optional)</span><textarea value={markingScheme} onChange={(event) => setMarkingScheme(event.target.value)} placeholder="Expected points or marks breakdown, if you have them..." rows={3} /></label></div>);
    }
  }
