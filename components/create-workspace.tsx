...
export function CreateWorkspace() {
  ...
  async function token() { const idToken = await getFirebaseServices().auth.currentUser?.getIdToken(); if (!idToken) throw new Error("Please log in again."); return idToken; }
  async function loadRecent() { try { const response = await fetch("/api/create", { headers: { Authorization: `Bearer ${await token()}` } }); const data = await response.json() as { creations?: Creation[]; error?: string }; if (!response.ok) throw new Error(data.error); setRecent(data.creations || []); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : "Unable to load creations."); } finally { setHistoryLoading(false); } }
  ...
  function continueInChat() { if (!creation) return; window.location.assign(`/chat?new=true&prompt=${encodeURIComponent(`Continue working on this ${creation.type} creation.\nRequest: ${creation.prompt}\nGenerated content:\n${creation.content}`)}`); }
