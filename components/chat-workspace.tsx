...
export function ChatWorkspace() {
  ...
  return <div className="chat-shell">
    <header className="chat-header"><div className="chat-header-leading"><Link className="icon-button chat-home-button" href="/" aria-label="Back to home" title="Back to home"><ArrowLeft size={19} /></Link><button className="icon-button mobile-back-button" type="button" onClick={leaveChat} aria-label="Go back" title="Go back"><ArrowLeft size={19} /></button><button className="icon-button chat-menu-button" type="button" onClick={() => setDrawerOpen(true)} aria-label="Open chat history"><Menu size={20} /></button><GoldAILogo compact /></div><div className="chat-header-actions"><ThemeToggle /><button className="new-chat-button" type="button" onClick={startNewChat}><Plus size={16} /> <span>New Chat</span></button></div></header>
