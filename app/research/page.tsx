...
function ResearchResult({ session, onContinue }: { session: ResearchSession; onContinue: () => void }) {
  return <section className="research-result"><div className="research-result-heading"><div><span className="eyebrow">Research result</span><h2>{session.title}</h2></div><span className="research-status">{session.status}</span></div><div className="research-result-text">{session.result}</div>{session.images.length > 0 && <div className="research-images">{session.images.map((image) => <a className="research-image-card" href={image.sourceUrl} target="_blank" rel="noreferrer" key={image.id}><img src={image.url} alt={image.alt} loading="lazy" /><span>{image.title}</span></a>)}</div>}<div className="research-actions"><button className="auth-submit" type="button" onClick={onContinue}>Discuss in Chat <ArrowRight size={16} /></button></div><div className="research-sources"><div className="section-title"><div><span className="eyebrow">Retrieved sources</span><h3>Sources</h3></div><span className="section-rule" /></div>{session.sources.map((source, index) => <SourceCard key={source.id} source={source} index={index} />)}</div></section>;
}
