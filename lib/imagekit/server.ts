...
export async function uploadToImageKit(file: File, userId: string, fileId: string) {
  ...
  const response = await fetch("https://upload.imagekit.io/api/v1/files/upload", { method: "POST", signal: AbortSignal.timeout(45_000), headers: { Authorization: `Basic ${Buffer.from(`${privateKey}:`).toString("base64")}` }, body: form });
  ...
  const fallbackThumbnailUrl = data.url.replace(/\/([^/]+)$/, "/tr:w-320/$1");
  return { imageKitFileId: data.fileId, url: data.url, thumbnailUrl: data.thumbnailUrl || fallbackThumbnailUrl };
}

export async function deleteFromImageKit(fileId: string) {
  const privateKey = required("IMAGEKIT_PRIVATE_KEY");
  const response = await fetch(`https://api.imagekit.io/v1/files/${encodeURIComponent(fileId)}`, { method: "DELETE", signal: AbortSignal.timeout(30_000), headers: { Authorization: `Basic ${Buffer.from(`${privateKey}:`).toString("base64")}` } });
